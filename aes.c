...
// possible GF(2^8) calculations.
// This table is only used by the (Inv)MixColumns steps.
// USAGE: The second index (column) is the coefficient
// of multiplication. Only 7 different
// coefficients are used:
// 0x01, 0x02, 0x03, 0x09, 0x0b, 0x0d, 0x0e,
// but multiplication by 1 is negligible leaving
// only 6 coefficients.
                        ...
                        int             keysize)
{
    ...
    // Perform the CBC operation with an IV of zeros
    // on the formatted buffer to calculate the MAC.
    ...
    // Last argument is the byte size of the counting
    ...
    aes_encrypt_ctr(&out[payload_len],
                    mac_len,
                    &out[payload_len],
                    key,
                    keysize,
